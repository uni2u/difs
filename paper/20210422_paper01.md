...
각 리전의 사용자들은 common name 을 사용하여 리전에 설정된 NDN 라우터를 통해 Chipmunk 저장소로 접근한다.
각 리전의 Chipmunk 노드는 클러스터를 구성하며 node name 으로 face 를 설정하여 연결한다.
...
Chipmunk 클러스터는 producer 의 네트워크 부담을 줄이기 위하여 manifest 방식을 제공한다.
