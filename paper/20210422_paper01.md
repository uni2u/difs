...
- ndn://{common name}/{command}/{RepoCommandParameter}
...
The data provider inserts the data into the repository using the common name and 'insert' commands in the repository.
The service-name of the data to be provided should not overlap in the Chipmunks network.
The data consumer receives data for the requested data name using the common name and 'get' commands. \par
All nodes that make up a Chipmunk cluster have common repository names, and each node has a node has its own node name.
