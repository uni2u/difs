...
However, the in-network caching alone is not sufficient for general data storage which is required in edge network environment \cite{icn}.
...
- NDN 기반의 persistent data storage 를 제공한다.

이 논문은 다음과 같이 구성된다.
섹션 2 는 related work 를 설명한다.
섹션 3 에서 우리는 전반적인 아키텍쳐와 상세한 절차를 설명한다.
섹션 4 는 우리가 제안한 결과의 성능을 분석한다.
섹션 5 는 향후 작업에 대하여 다루고 있으며 섹션 6 은 결론을 내린다.
...
DIFS 노드는 데이터 패킷과 데이터의 메타데이터인 manifest 파일을 저장한다.
...
DIFS 관리자는 keyspace 파일을 생성하는 것으로 저장 서비스를 사용하는 리전이 추가되거나 특정 keyspace 범위의 파일이 증가하는 경우 저장 노드를 추가할 수 있다.
...
이러한 방식을 통하여 DIFS 모든 노드는 같은 keyspace 파일을 가진다.
...
특히, NDNS 및 NLSR 이 포함된 경우 producer 는 NDNS 와 NLSR 에 직접 등록하여야 한다.
현재 NDNS 의 경우 tree 구조를 가지고 있으며 top tree 에 등록이 되기까지 상당한 시간을 대기하여야 하며 NLSR 의 경우 sync protocol 을 통하여 모든 NLSR 의 동기화가 완료되어야 정상동작하게 된다.
DIFS 클러스터는 producer 의 네트워크 부담을 줄이기 위하여 manifest 방식을 제공한다.
